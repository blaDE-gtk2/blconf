...
static void
blconf_perl_ghashtable_to_hv(gpointer key,
                             gpointer valuep,
                             gpointer data)
{
...
MODULE = Xfce4::Blconf::Channel    PACKAGE = Xfce4::Blconf::Channel    PREFIX = blconf_channel_
PROTOTYPES: ENABLE

BlconfChannel *
blconf_channel_new(class, channel_name)
        const gchar * channel_name
    C_ARGS:
        channel_name

BlconfChannel *
blconf_channel_new_with_property_base(class, channel_name, property_base)
        const gchar * channel_name
...
gboolean
blconf_channel_has_property(channel, property)
        BlconfChannel * channel
        const gchar * property

gboolean
blconf_channel_is_property_locked(channel, property)
        BlconfChannel * channel
        const gchar * property

void
blconf_channel_reset_property(channel, property_base, recursive=FALSE)
        BlconfChannel * channel
        const gchar * property_base
		gboolean recursive

HV *
blconf_channel_get_properties(channel, property_base=NULL)
        BlconfChannel * channel
        const gchar * property_base
    PREINIT:
        GHashTable *properties = NULL;
    CODE:
        properties = blconf_channel_get_properties(channel, property_base);
        if(!properties)
            RETVAL = (HV *)&PL_sv_undef;
        else {
            RETVAL = newHV();
            g_hash_table_foreach(properties,
                                 blconf_perl_ghashtable_to_hv,
                                 RETVAL);
...
void
blconf_channel_get_property(channel, property, default_value=NULL)
        BlconfChannel * channel
        const gchar * property
        SV * default_value
    PREINIT:
        GValue val = { 0, };
    PPCODE:
        if(!blconf_channel_get_property(channel, property, &val)) {
            XPUSHs((default_value ? default_value : &PL_sv_undef));
        } else if(G_VALUE_TYPE(&val) == BLCONF_TYPE_G_VALUE_ARRAY) {
...
_set_property(channel, property, value, arraytypes=NULL)
    ...
    CODE:
        ...
        switch(ix) {
            ...
            case 2:  gtype = BLCONF_TYPE_UINT16; break;
            case 3:  gtype = BLCONF_TYPE_INT16; break;
            ...
            case 12: gtype = BLCONF_TYPE_G_VALUE_ARRAY; break;
        }

        if(gtype != BLCONF_TYPE_G_VALUE_ARRAY) {
            g_value_init(&val, gtype);
            gperl_value_from_sv(&val, value);
            RETVAL = blconf_channel_set_property(channel, property, &val);
            g_value_unset(&val);
        } else {
            GPtrArray *arr;
            ...
            if(!SvROK(value) || (arraytypes && !SvROK(arraytypes)))
                croak("Usage: Xfce4::Blconf::Channel::set_array($property, \\@values, \\@types=NULL)");

            av_values = (AV *)SvRV(value);
            av_types = arraytypes ? (AV *)SvRV(arraytypes) : NULL;

            if(av_len(av_values) != av_len(av_types))
                croak("Xfce4::Blconf::Channel::set_array(): values array is not the same size as the types array");
            ...
            for(i = 0; i < num_elements; ++i) {
                ...
                if(!val_sv || !*val_sv || !SvOK(*val_sv) || SvROK(*val_sv)) {
                    ...
                    croak("Xfce4::Blconf::Channel::set_array(): invalid value at index %d", i);
                }

                if(av_types) {
                    SV **type_sv = av_fetch(av_types, i, 0);
                    if(type_sv && *type_sv && SvOK(*type_sv))
                        gtype = _blconf_gtype_from_string(SvGChar(*type_sv));
                }

                if(gtype == G_TYPE_INVALID) {
                    if(av_types)
                        warn("Xfce4::Blconf::Channel::set_array(): unable to determine type at index %d; guessing", i);
                    if(SvNOKp(*val_sv))
                        gtype = G_TYPE_DOUBLE;
                ...
                if(gtype == G_TYPE_NONE || gtype == BLCONF_TYPE_G_VALUE_ARRAY) {
                    blconf_array_free(arr);
                    croak("Xfce4::Blconf::Channel::set_array(): value cannot be of type 'empty' or 'array' at index %d", i);
                }
            ...
            RETVAL = blconf_channel_set_arrayv(channel, property, arr);
            blconf_array_free(arr);
        }
