...
static void blconf_g_property_object_notify(GObject *object,
                                            GParamSpec *pspec,
                                            gpointer user_data);
static void blconf_g_property_object_disconnect(gpointer user_data,
                                                GClosure *closure);
static void blconf_g_property_channel_notify(BlconfChannel *channel,
                                             const gchar *property,
                                             const GValue *value,
                                             gpointer user_data);
static void blconf_g_property_channel_disconnect(gpointer user_data,
                                                 GClosure *closure);
...
static void
blconf_g_property_object_notify_gdkcolor(BlconfGBinding *binding)
{
    ...
    blconf_channel_set_array(binding->channel, binding->blconf_property,
...
static void
blconf_g_property_object_notify_gdkrgba(BlconfGBinding *binding)
{
...
static void
blconf_g_property_object_notify(GObject *object,
                                GParamSpec *pspec,
                                gpointer user_data)
{
    BlconfGBinding *binding = user_data;
    GValue src_val = { 0, };
    GValue dst_val = { 0, };

    g_return_if_fail(G_IS_OBJECT(object));
    g_return_if_fail(binding->object == object);
    g_return_if_fail(BLCONF_IS_CHANNEL(binding->channel));

    if(G_PARAM_SPEC_VALUE_TYPE(pspec) == __gdkcolor_gtype) {
        /* we need to handle this in a different way */
        blconf_g_property_object_notify_gdkcolor(binding);
        return;
    }

    if(G_PARAM_SPEC_VALUE_TYPE(pspec) == __gdkrgba_gtype) {
        /* we need to handle this in a different way */
        blconf_g_property_object_notify_gdkrgba(binding);
        return;
    }
    ...
    g_value_init(&dst_val, binding->blconf_property_type);
    if(g_value_transform(&src_val, &dst_val)) {
        g_signal_handler_block(G_OBJECT(binding->channel),
                               binding->channel_handler);
        blconf_channel_set_property(binding->channel,
                                    binding->blconf_property,
                                    &dst_val);
...
static void
blconf_g_property_object_disconnect(gpointer user_data,
                                    GClosure *closure)
{
    BlconfGBinding *binding = user_data;

    g_return_if_fail(G_IS_OBJECT(binding->object));
    g_return_if_fail(!binding->channel || BLCONF_IS_CHANNEL(binding->channel));
    ...
    g_free(binding->blconf_property);
    g_free(binding->object_property);
    g_slice_free(BlconfGBinding, binding);
}

static void
blconf_g_property_channel_notify_gdkcolor(BlconfGBinding *binding,
                                          const GValue *value)
{
    GPtrArray *arr;
...
static void
blconf_g_property_channel_notify_gdkrgba(BlconfGBinding *binding,
                                         const GValue *value)
{
    GPtrArray *arr;
    FakeGdkRGBA color = { 0, };
...
static void
blconf_g_property_channel_notify(BlconfChannel *channel,
                                 const gchar *property,
                                 const GValue *value,
                                 gpointer user_data)
{
   ...
   if(__gdkcolor_gtype == binding->blconf_property_type) {
       /* we need to handle this in a different way */
        blconf_g_property_channel_notify_gdkcolor(binding, value);
        return;
    }

    if(__gdkrgba_gtype == binding->blconf_property_type) {
       /* we need to handle this in a different way */
        blconf_g_property_channel_notify_gdkrgba(binding, value);
        return;
    }
...
static void
blconf_g_property_channel_disconnect(gpointer user_data,
                                     GClosure *closure)
{
    BlconfGBinding *binding = user_data;
...
static gulong
blconf_g_property_init(BlconfChannel *channel,
                       ...
                       GType object_property_type)
{
    BlconfGBinding *binding;
    gchar *detailed_signal;
    GValue value = { 0, };

    binding = g_slice_new(BlconfGBinding);
    binding->channel = channel;
    binding->blconf_property_type = blconf_property_type;
    binding->blconf_property = g_strdup(blconf_property);
    ...
    binding->object_handler = g_signal_connect_data(G_OBJECT(object),
                                                    detailed_signal,
                                                    G_CALLBACK(blconf_g_property_object_notify),
                                                    binding,
                                                    blconf_g_property_object_disconnect, 0);
    g_free(detailed_signal);

    /* transfer channel property to the object */
    if(blconf_channel_get_property(channel, blconf_property, &value)) {
        blconf_g_property_channel_notify(channel, blconf_property,
                                         &value, binding);
        g_value_unset(&value);
    }

    /* monitor channel for property changes */
    detailed_signal = g_strconcat("property-changed::", blconf_property, NULL);
    binding->channel_handler = g_signal_connect_data(G_OBJECT(channel),
                                                     detailed_signal,
                                                     G_CALLBACK(blconf_g_property_channel_notify),
                                                     binding,
                                                     blconf_g_property_channel_disconnect, 0);
    g_free(detailed_signal);
...
void
_blconf_g_bindings_shutdown(void)
{
    GSList *bindings, *l;
    guint n;
    BlconfGBinding *binding;
    ...
    if(G_UNLIKELY(__bindings)) {
        ...
        g_debug("%d blconf binding(s) are still connected. Are you sure all blconf "
                "channels are released before calling blconf_shutdown()?", n);
#endif
...
/**
 * blconf_g_property_bind:
 * @channel: An #BlconfChannel.
 * @blconf_property: A property on @channel.
 * @blconf_property_type: The type of @blconf_property.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 *
 * Binds an Blconf property to a #GObject property.  If the property
 * is changed via either the #GObject or Blconf, the corresponding
 * property will also be updated.
 *
 * Note that @blconf_property_type is required since @blconf_property
 * may or may not already exist in the Blconf store.  The type of
 ...
 **/
gulong
blconf_g_property_bind(BlconfChannel *channel,
                       const gchar *blconf_property,
                       GType blconf_property_type,
                       gpointer object,
                       const gchar *object_property)
{
    ...
    g_return_val_if_fail(BLCONF_IS_CHANNEL(channel), 0UL);
    g_return_val_if_fail(blconf_property && *blconf_property == '/', 0UL);
    g_return_val_if_fail(blconf_property_type != G_TYPE_NONE, 0UL);
    g_return_val_if_fail(blconf_property_type != G_TYPE_INVALID, 0UL);
    ...
    if(G_UNLIKELY(!g_value_type_transformable(blconf_property_type,
    ...
    return blconf_g_property_init(channel, blconf_property,
                                  blconf_property_type, G_OBJECT(object),
                                  object_property,
                                  G_PARAM_SPEC_VALUE_TYPE(pspec));
}

/**
 * blconf_g_property_bind_gdkcolor:
 * @channel: An #BlconfChannel.
 * @blconf_property: A property on @channel.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 *
 * Binds an Blconf property to a #GObject property of type
 ...
 * Blconf store as four 16-bit unsigned ints (red, green, blue, alpha).
 ...
 **/
gulong
blconf_g_property_bind_gdkcolor(BlconfChannel *channel,
                                const gchar *blconf_property,
                                gpointer object,
                                const gchar *object_property)
{
...
/**
 * blconf_g_property_bind_gdkrgba:
 * @channel: An #BlconfChannel.
 * @blconf_property: A property on @channel.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 ...
 **/
gulong
blconf_g_property_bind_gdkrgba(BlconfChannel *channel,
                               const gchar *blconf_property,
                               gpointer object,
                               const gchar *object_property)
{
 ...
 *
 * Removes an Blconf/GObject property binding based on the binding
 * ID number.  See blconf_g_property_bind().
 **/
void
blconf_g_property_unbind(gulong id)
{
    GSList *l;
    BlconfGBinding *binding;
...
/**
 * blconf_g_property_unbind_by_property:
 * @channel: An #BlconfChannel.
 * @blconf_property: A bound property on @channel.
 * @object: A #GObject.
 * @object_property: A bound property on @object.
 *
 * Causes an Blconf channel previously bound to a #GObject property
 * (see blconf_g_property_bind()) to no longer be bound.
 **/
void
blconf_g_property_unbind_by_property(BlconfChannel *channel,
                                     const gchar *blconf_property,
                                     gpointer object,
                                     const gchar *object_property)
{
    ...
    g_return_if_fail(BLCONF_IS_CHANNEL(channel));
    g_return_if_fail(blconf_property && *blconf_property == '/');
    g_return_if_fail(G_IS_OBJECT(object));
    ...
    for(l = __bindings; l; l = g_slist_next(l)) {
        ...
        if(binding->object == object
           && binding->channel == channel
           && !strcmp(blconf_property, binding->blconf_property)
...
/**
 * blconf_g_property_unbind_all:
 * @channel_or_object: A #GObject or #BlconfChannel.
 *
 * Unbinds all Blconf channel bindings (see blconf_g_property_bind())
 * to @object.  If @object is an #BlconfChannel, it will unbind all
 * blconf properties on that channel.  If @object is a regular #GObject
 * with properties bound to a channel, all those bindings will be
 * removed.
 **/
void
blconf_g_property_unbind_all(gpointer channel_or_object)
{
    guint n;

    g_return_if_fail(G_IS_OBJECT(channel_or_object));

    if(BLCONF_IS_CHANNEL(channel_or_object)) {
        n = g_signal_handlers_disconnect_matched(channel_or_object, G_SIGNAL_MATCH_FUNC,
                                                 0, 0, NULL,
                                                 blconf_g_property_channel_notify,
                                                 NULL);
    } else {
        n = g_signal_handlers_disconnect_matched(channel_or_object, G_SIGNAL_MATCH_FUNC,
                                                 0, 0, NULL,
                                                 blconf_g_property_object_notify,
                                                 NULL);
    }

    if(G_UNLIKELY(!n)) {
        g_warning("No bindings were found on the %s",
                  BLCONF_IS_CHANNEL(channel_or_object) ? "channel" : "object");
    }
}



#define __BLCONF_BINDING_C__
