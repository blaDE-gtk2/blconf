...
#define HANDLE_TYPE(gtype_s, getter) \
    ...
    if(G_VALUE_TYPE(dest_value) == BLCONF_TYPE_UINT16) {
        if(dest > USHRT_MAX) {
            g_warning("Converting type \"%s\" to \"%s\" results in overflow",
                      G_VALUE_TYPE_NAME(src_value),
                      G_VALUE_TYPE_NAME(dest_value));
        }
        blconf_g_value_set_uint16(dest_value, (guint16)dest);
    } else if(G_VALUE_TYPE(dest_value) == BLCONF_TYPE_INT16) {
        if(dest > (guint64) SHRT_MAX || dest < (guint64) SHRT_MIN) {
            g_warning("Converting type \"%s\" to \"%s\" results in overflow",
                      G_VALUE_TYPE_NAME(src_value),
                      G_VALUE_TYPE_NAME(dest_value));
        }
        blconf_g_value_set_int16(dest_value, (gint16)dest);
    }
#undef HANDLE_TYPE
}
...
#define HANDLE_TYPE(gtype_s, setter) \
    ...
    if(G_VALUE_TYPE(src_value) == BLCONF_TYPE_UINT16)
        src = blconf_g_value_get_uint16(src_value);
    else if(G_VALUE_TYPE(src_value) == BLCONF_TYPE_INT16) {
        src = blconf_g_value_get_int16(src_value);
        is_signed = TRUE;
    } else
        return;
 ...
 **/
guint16
blconf_g_value_get_uint16(const GValue *value)
{
    g_return_val_if_fail(G_VALUE_HOLDS(value, BLCONF_TYPE_UINT16), 0);
    return (guint16)value->data[0].v_int;
}

/**
 * blconf_g_value_set_uint16:
 * @value: A #GValue.
 * @v_uint16: A guint16.
 *
 ...
 **/
void
blconf_g_value_set_uint16(GValue *value,
                          guint16 v_uint16)
{
    g_return_if_fail(G_VALUE_HOLDS(value, BLCONF_TYPE_UINT16));
    value->data[0].v_int = v_uint16;
}

GType
blconf_int16_get_type(void)
{
 ...
 **/
gint16
blconf_g_value_get_int16(const GValue *value)
{
    g_return_val_if_fail(G_VALUE_HOLDS(value, BLCONF_TYPE_INT16), 0);
    return (gint16)value->data[0].v_int;
}

/**
 * blconf_g_value_set_int16:
 * @value: A #GValue.
 * @v_int16: A gint16.
 *
 ...
 **/
void
blconf_g_value_set_int16(GValue *value,
                         gint16 v_int16)
{
    g_return_if_fail(G_VALUE_HOLDS(value, BLCONF_TYPE_INT16));
    value->data[0].v_int = v_int16;
}


#ifdef LIBBLCONF_COMPILATION
#define __BLCONF_TYPES_C__
