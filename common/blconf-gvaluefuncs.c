...
#include "blconf-gvaluefuncs.h"
#include "blconf/blconf-types.h"
#include "blconf-common-private.h"

#ifdef CHAR_MIN
#define BLCONF_MINCHAR  CHAR_MIN
#else 
#define BLCONF_MINCHAR  (-128)
#endif

#ifdef CHAR_MAX
#define BLCONF_MAXCHAR  CHAR_MAX
#else
#define BLCONF_MAXCHAR  (127)
#endif

#ifdef UCHAR_MAX
#define BLCONF_MAXUCHAR  UCHAR_MAX
#else
#define BLCONF_MAXUCHAR  (255)
#endif

GType
_blconf_gtype_from_string(const gchar *type)
{
    ...
    if(!strcmp(type, "empty"))
        ...
        return BLCONF_TYPE_G_VALUE_ARRAY;
        ...
        return BLCONF_TYPE_UINT16;
    else if(!strcmp(type, "int16"))
        return BLCONF_TYPE_INT16;
...
const gchar *
_blconf_string_from_gtype(GType gtype)
{
    switch(gtype) {
        case G_TYPE_STRING:
            return "string";
        ...
        case G_TYPE_BOOLEAN:
            return "bool";
        default:
            if(gtype == BLCONF_TYPE_UINT16)
                return "uint16";
            else if(gtype == BLCONF_TYPE_INT16)
                return "int16";
            else if(gtype == BLCONF_TYPE_G_VALUE_ARRAY)
                return "array";
            break;
    }

    g_warning("GType %s doesn't map to an Blconf type",
              g_type_name(gtype));
    return NULL;
}

gboolean
_blconf_gvalue_from_string(GValue *value,
                           const gchar *str)
{
...
#define HANDLE_INT(minval, maxval, setfunc)  REAL_HANDLE_INT(minval, maxval, strtol, setfunc)
    ...
    switch(G_VALUE_TYPE(value)) {
        ...
        case G_TYPE_UCHAR:
            HANDLE_UINT(0, BLCONF_MAXUCHAR, g_value_set_uchar);
        case G_TYPE_CHAR:
#if GLIB_CHECK_VERSION (2, 32, 0)
            HANDLE_INT(G_MININT8, G_MAXINT8, g_value_set_schar);
#else
            HANDLE_INT(BLCONF_MINCHAR, BLCONF_MAXCHAR, g_value_set_char);
#endif
        case G_TYPE_UINT:
        ...
        default:
            if(BLCONF_TYPE_UINT16 == G_VALUE_TYPE(value)) {
                HANDLE_INT(0, G_MAXUSHORT, blconf_g_value_set_uint16);
                return TRUE;
            } else if(BLCONF_TYPE_INT16 == G_VALUE_TYPE(value)) {
                HANDLE_INT(G_MINSHORT, G_MAXSHORT, blconf_g_value_set_int16);
                return TRUE;
...
gchar *
_blconf_string_from_gvalue(GValue *val)
{
    ...
    switch(G_VALUE_TYPE(val)) {
        ...
        default:
            if(G_VALUE_TYPE(val) == BLCONF_TYPE_UINT16) {
                return g_strdup_printf("%u",
                                       (guint)blconf_g_value_get_uint16(val));
            } else if(G_VALUE_TYPE(val) == BLCONF_TYPE_INT16) {
                return g_strdup_printf("%d",
                                       (gint)blconf_g_value_get_int16(val));
            }
            break;
    }
...
gboolean
_blconf_gvalue_is_equal(const GValue *value1,
                        const GValue *value2)
{
...
#define HANDLE_CMP_GV(TYPE, getter) \
        ...
        default:
            if(G_VALUE_TYPE(value1) == BLCONF_TYPE_INT16)
                return blconf_g_value_get_int16(value1) == blconf_g_value_get_uint16(value2);
            else if(G_VALUE_TYPE(value1) == BLCONF_TYPE_UINT16)
                return blconf_g_value_get_uint16(value1) == blconf_g_value_get_uint16(value2);
            break;
#undef HANDLE_CMP_GV
    }
...
void
_blconf_gvalue_free(GValue *value)
{
    if(!value)
        return;
    g_value_unset(value);
