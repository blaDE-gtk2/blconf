...
static void
blconf_query_monitor (BlconfChannel *channel, const gchar *changed_property, GValue *property_value)
{
    gchar *string;
    ...
    if(property_value && G_IS_VALUE (property_value))
    {
        if(verbose)
        {
             string = _blconf_string_from_gvalue(property_value);
...
static void
blconf_query_get_propname_size (gpointer key, gpointer value, gpointer user_data)
{
    gint *size = user_data;
...
static void
blconf_query_list_sorted (gpointer key, gpointer value, gpointer user_data)
{
...
static void
blconf_query_list_contents (GSList *sorted_contents, GHashTable *channel_contents, gint size)
{
    GSList *li;
    ...
    for (li = sorted_contents; li != NULL; li = li->next)
    {
        if (verbose)
        {
            ...
            if (BLCONF_TYPE_G_VALUE_ARRAY != G_VALUE_TYPE (property_value))
            {
                string = _blconf_string_from_gvalue (property_value);
            }
            else
...
static void
blconf_query_printerr(const gchar *message,
                      ...)
{
    va_list args;
    gchar *str;
...
main(int argc, char **argv)
{
    ...
    {
        blconf_query_printerr(_("Failed to init libblconf: %s"), error->message);
        g_error_free(error);
        return EXIT_FAILURE;
    }

    context = g_option_context_new(_("- Blconf commandline utility"));
    g_option_context_add_main_entries(context, entries, GETTEXT_PACKAGE);

    if(!g_option_context_parse(context, &argc, &argv, &error))
    {
        blconf_query_printerr(_("Option parsing failed: %s"), error->message);
        g_error_free(error);
        return EXIT_FAILURE;
    }
    ...
    {
        blconf_query_printerr(_("No property specified"));
        return EXIT_FAILURE;
    }

    if (create && reset)
    {
        blconf_query_printerr(_("--create and --reset options can not be used together"));
        return EXIT_FAILURE;
    }

    if ((create || reset) && (list))
    {
        blconf_query_printerr(_("--create and --reset options can not be used together with --list"));
        return EXIT_FAILURE;
    }

    channel = blconf_channel_new(channel_name);

    if (monitor)
    {
        GMainLoop *loop;

        g_signal_connect (G_OBJECT (channel), "property-changed", G_CALLBACK (blconf_query_monitor), NULL);
    ...
    if (property_name && !list)
    {
        ...
        if (reset)
        {
            blconf_channel_reset_property(channel, property_name, recursive);
        }
        /** Read value */
        else if(set_value == NULL || set_value[0] == NULL)
        {
            ...
            if(!blconf_channel_get_property(channel, property_name, &value))
            {
                blconf_query_printerr(_("Property \"%s\" does not exist on channel \"%s\""),
            ...
            if (toggle)
            {
                ...
                {
                    if(blconf_channel_set_bool(channel, property_name, !g_value_get_boolean(&value)))
                        return EXIT_SUCCESS;
                    else
                        blconf_query_printerr(_("Failed to set property"));
                }
                else
                {
                    blconf_query_printerr(_("--toggle only works with boolean values"));
                }

                return EXIT_FAILURE;
            }

            if(BLCONF_TYPE_G_VALUE_ARRAY != G_VALUE_TYPE(&value))
            {
                gchar *str_val = _blconf_string_from_gvalue(&value);
            ...
            else
            {
                ...
                for(i = 0; i < arr->len; ++i)
                {
                    ...
                    if(item_value)
                    {
                        gchar *str_val = _blconf_string_from_gvalue(item_value);
                        g_print("%s\n", str_val ? str_val : _("(unknown)"));
                        g_free(str_val);
                    }
                }
        ...
        /* Write value */
            ...
            prop_exists = blconf_channel_has_property(channel, property_name);
            if(!prop_exists && !create)
            {
                blconf_query_printerr(_("Property \"%s\" does not exist on channel \"%s\". If a new "
            ...
            {
                blconf_query_printerr(_("When creating a new property, the value type must be specified"));
                return EXIT_FAILURE;
            }
            ...
            if(prop_exists && !(type && type[0]))
            {
                ...
                if(!blconf_channel_get_property(channel, property_name, &value))
                {
                    blconf_query_printerr(_("Failed to get the existing type for the value"));
                    return EXIT_FAILURE;
                }
            }
            ...
            if(!set_value[1] && !force_array)
            {
                ...
                if(type && type[0])
                    gtype = _blconf_gtype_from_string(type[0]);

                if(G_TYPE_INVALID == gtype || G_TYPE_NONE == gtype)
                {
                    blconf_query_printerr(_("Unable to determine the type of the value"));
                    return EXIT_FAILURE;
                }

                if(BLCONF_TYPE_G_VALUE_ARRAY == gtype)
                {
                    blconf_query_printerr(_("A value type must be specified to change an array into a single value"));
                    return EXIT_FAILURE;
                }
                ...
                if(!_blconf_gvalue_from_string(&value, set_value[0]))
                {
                    blconf_query_printerr(_("Unable to convert \"%s\" to type \"%s\""),
                                          set_value[0], g_type_name(gtype));
                    return EXIT_FAILURE;
                }

                if(!blconf_channel_set_property(channel, property_name, &value))
                {
                    blconf_query_printerr(_("Failed to set property"));
                    return EXIT_FAILURE;
                }
            ...
            else
            {
                ...
                {
                    blconf_query_printerr(_("There are %d new values, but only %d types could be determined"),
                ...
                for(i = 0; set_value[i]; ++i) {
                    ...
                    if(arr_old)
                    {
                        ...
                        gtype = _blconf_gtype_from_string(type[i]);

                    if(G_TYPE_INVALID == gtype || G_TYPE_NONE == gtype)
                    {
                        blconf_query_printerr(_("Unable to determine type of value at index %d"), i);
                        return EXIT_FAILURE;
                    }

                    value_new = g_new0(GValue, 1);
                    g_value_init(value_new, gtype);
                    if(!_blconf_gvalue_from_string(value_new, set_value[i]))
                    {
                        blconf_query_printerr(_("Unable to convert \"%s\" to type \"%s\""),
    ...
    if (list)
    {
        GHashTable *channel_contents = blconf_channel_get_properties(channel, property_name);
        if (channel_contents)
        {
            gint size = 0;
            ...
            if (verbose)
                g_hash_table_foreach (channel_contents, (GHFunc)blconf_query_get_propname_size, &size);

            g_hash_table_foreach (channel_contents, (GHFunc)blconf_query_list_sorted, &sorted_contents);

            blconf_query_list_contents (sorted_contents, channel_contents, size);
